...
(function () {
    ...
    // ==================== CONFIGURACION DE GRUPOS DE MUELLES ====================
    // Cada grupo tiene reglas por tipo de paquete. Para cada campo:
    //   min = el valor debe ser >= min  (mayor o igual)
    //   max = el valor debe ser <= max  (menor o igual)
    //   Si ambos estan, es "entre min y max"
    //   Si esta vacio, no se comprueba ese campo
    //
    // Campos en %: xss, m, l, xl, nc, ncp (porcentaje sobre total de paquetes)
    // Campos absolutos: fluid, xdock, total
    //
    // Los grupos se evaluan en orden de prioridad (1 = primero).
    // El primer grupo donde TODAS las condiciones se cumplen, gana.
    // Si ningun grupo coincide, fallback = 300b.

    const FIELDS = [
        { key: 'xss',   label: 'XS+S',   unit: '%' },
        { key: 'm',     label: 'M',       unit: '%' },
        { key: 'l',     label: 'L',       unit: '%' },
        { key: 'xl',    label: 'XL',      unit: '%' },
        { key: 'nc',    label: 'NC',      unit: '%' },
        { key: 'ncp',   label: 'NC+',     unit: '%' },
        { key: 'fluid', label: 'Fluid',   unit: 'abs' },
        { key: 'xdock', label: 'XDock',   unit: 'abs' },
        { key: 'total', label: 'Total',   unit: 'abs' },
    ];

    // Reglas por defecto (replicando la logica original)
    const DEFAULT_GROUP_RULES = {
        '100x':   { priority: 1, docks: 'DD-134, DD-135, DD-136', rules: { nc: { min: 80 }, ncp: { min: 0 } } },
        'fluidsA':{ priority: 2, docks: 'DD-233, DD-232, DD-231', rules: { fluid: { min: 1, max: 200 } } },
        'fluidsB':{ priority: 3, docks: 'DD-229, DD-228, DD-227', rules: { fluid: { min: 201, max: 600 } } },
        'fluidsC':{ priority: 4, docks: 'DD-227, DD-226, DD-225', rules: { fluid: { min: 601 } } },
        '300c':   { priority: 5, docks: 'DD-303, DD-302, DD-301', rules: { xdock: { min: 10 }, xss: { min: 65 } } },
        '200a':   { priority: 6, docks: 'DD-239, DD-238, DD-237', rules: { m: { min: 15 }, xdock: { min: 8 } } },
        '200b':   { priority: 7, docks: 'DD-236, DD-235, DD-234', rules: { m: { min: 15 } } },
        '300a':   { priority: 8, docks: 'DD-309, DD-308, DD-307', rules: { xss: { min: 60 }, total: { min: 3000 } } },
        '300b':   { priority: 9, docks: 'DD-306, DD-305, DD-304', rules: { xss: { min: 60 } } },
    };

    function loadGroupRules() {
        try {
            const raw = GM_getValue('inbGroupRules', null);
    ...
    function saveGroupRules(rules) {
        try { GM_setValue('inbGroupRules', JSON.stringify(rules)); } catch (e) {}
    }

    function decideGroup(v) {
        const rules = loadGroupRules();
        const total = v.total || 0;
        const pctOf = (x) => total > 0 ? (x / total) * 100 : 0;

        // Calcular porcentajes y absolutos
        const vals = {
            xss:   pctOf(v.xss || 0),
            m:     pctOf(v.m || 0),
            l:     pctOf(v.l || 0),
            xl:    pctOf(v.xl || 0),
            nc:    pctOf((v.nc || 0) + (v.ncp || 0)),  // NC incluye NC+ para simplificar
            ncp:   pctOf(v.ncp || 0),
            fluid: v.fluid || 0,
            xdock: v.xdock || 0,
            total: total,
        ...
        for (const group of sorted) {
            if (!group.rules || Object.keys(group.rules).length === 0) continue;
            let allMatch = true;
            const reasons = [];

            for (const [field, cond] of Object.entries(group.rules)) {
                const val = vals[field];
                if (val === undefined) { allMatch = false; break; }

                const hasMin = cond.min !== undefined && cond.min !== null && cond.min !== '';
                const hasMax = cond.max !== undefined && cond.max !== null && cond.max !== '';

                if (hasMin && val < parseFloat(cond.min)) { allMatch = false; break; }
                if (hasMax && val > parseFloat(cond.max)) { allMatch = false; break; }

                const unit = FIELDS.find(f => f.key === field)?.unit === '%' ? '%' : '';
                if (hasMin && hasMax) reasons.push(`${field}: ${val.toFixed(0)}${unit} [${cond.min}-${cond.max}]`);
                else if (hasMin) reasons.push(`${field}: ${val.toFixed(0)}${unit} ≥${cond.min}`);
                else if (hasMax) reasons.push(`${field}: ${val.toFixed(0)}${unit} ≤${cond.max}`);
            }

            if (allMatch) return { group: group.name, reason: reasons.join(', ') };
        }

        // Fallback
        return { group: '300b', reason: 'Sin match (fallback)' };
    }
    ...
    function initSocrates() {
        ...
        style.textContent = `
            ...
            .bcn8-s-cfg-toggle {
                ...
                background: #eef; border: 1px solid #ccd; border-radius: 5px;
                font-size: 11px; font-weight: 600; color: #335; cursor: pointer;
                text-align: left;
            }
            .bcn8-s-cfg-toggle:hover { background: #ddf; }
            .bcn8-s-cfg {
                display: none; margin-top: 6px; padding: 8px;
                background: #f8f9ff; border: 1px solid #dde; border-radius: 6px;
            }
            .bcn8-s-cfg.open { display: block; }
            .bcn8-s-cfg table { width: 100%; border-collapse: collapse; font-size: 10px; }
            .bcn8-s-cfg th { background: #1d5d9b; color: #fff; padding: 4px 6px; text-align: left; font-size: 10px; }
            .bcn8-s-cfg td { padding: 3px 6px; border-bottom: 1px solid #eef; }
            .bcn8-s-cfg input[type="number"] {
                width: 55px; padding: 2px 4px; border: 1px solid #ccd; border-radius: 3px;
                font-size: 11px; font-family: 'Consolas', monospace; text-align: right;
            }
            .bcn8-s-cfg-save {
                margin-top: 6px; padding: 5px 14px; background: #1b5e20; color: #fff;
                border: none; border-radius: 4px; font-size: 11px; font-weight: 700;
                cursor: pointer;
            }
            .bcn8-s-cfg-save:hover { background: #2e7d32; }
            .bcn8-s-cfg-ok { color: #1b5e20; font-size: 10px; font-weight: 600; display: none; margin-left: 8px; }
        `;
        document.head.appendChild(style);
        ...
        panel.innerHTML = `
            ...
            <div class="bcn8-s-body" id="bcn8-s-body">
                ...
                <button class="bcn8-s-cfg-toggle" id="bcn8-s-cfg-toggle">⚙️ Configurar grupos de muelles</button>
                <div class="bcn8-s-cfg" id="bcn8-s-cfg">
                    <div style="font-size:10px;color:#666;margin-bottom:6px;line-height:1.4;">
                        Para cada grupo, configura <b>min (≥)</b> y/o <b>max (≤)</b> de cada tipo de paquete.<br/>
                        Campos en % = porcentaje sobre Total Packages. Fluid/XDock/Total = valores absolutos.<br/>
                        Deja vacío = no se comprueba. Se evalúan en orden de prioridad (1 primero).
                    </div>
                    <div id="bcn8-s-cfg-groups"></div>
                    <div style="margin-top:8px;display:flex;align-items:center;gap:8px;">
                        <button class="bcn8-s-cfg-save" id="bcn8-s-cfg-save">💾 Guardar</button>
                        <button class="bcn8-s-cfg-save" id="bcn8-s-cfg-reset" style="background:#b71c1c;">🔄 Reset</button>
                        <span class="bcn8-s-cfg-ok" id="bcn8-s-cfg-ok">✓ Guardado</span>
                    </div>
                </div>
            </div>
        `;
        ...
        // ====== Panel configurable de grupos de muelles ======
        const cfgToggle = panel.querySelector('#bcn8-s-cfg-toggle');
        const cfgPanel  = panel.querySelector('#bcn8-s-cfg');
        const cfgGroups = panel.querySelector('#bcn8-s-cfg-groups');
        const cfgSave   = panel.querySelector('#bcn8-s-cfg-save');
        const cfgReset  = panel.querySelector('#bcn8-s-cfg-reset');
        const cfgOk     = panel.querySelector('#bcn8-s-cfg-ok');

        cfgToggle.addEventListener('click', () => {
            cfgPanel.classList.toggle('open');
            cfgToggle.textContent = cfgPanel.classList.contains('open')
                ? '⚙️ Ocultar configuración de grupos'
                : '⚙️ Configurar grupos de muelles';
        });

        const GROUP_COLORS = {
            '100x':'#6a1b9a', 'fluidsA':'#1b5e20', 'fluidsB':'#2e7d32', 'fluidsC':'#388e3c',
            '300c':'#e65100', '200a':'#0d47a1', '200b':'#1565c0', '300a':'#f57c00', '300b':'#ff8f00',
        };

        function renderCfgUI() {
            const rules = loadGroupRules();
            let html = '';
            const groupOrder = Object.entries(rules).sort((a,b) => (a[1].priority||99) - (b[1].priority||99));

            for (const [gName, gCfg] of groupOrder) {
                const color = GROUP_COLORS[gName] || '#333';
                const docks = gCfg.docks || GROUP_DOCKS[gName]?.join(', ') || '';
                html += `<div style="margin-bottom:8px;border:1px solid #dde;border-radius:6px;overflow:hidden;">`;
                html += `<div style="background:${color};color:#fff;padding:4px 8px;font-size:11px;font-weight:700;display:flex;justify-content:space-between;align-items:center;">`;
                html += `<span>${gName} — ${docks}</span>`;
                html += `<span style="font-weight:400;font-size:10px;">Prio: <input type="number" data-group="${gName}" data-field="__priority" value="${gCfg.priority||99}" min="1" max="20" style="width:30px;font-size:10px;text-align:center;border:none;border-radius:2px;padding:1px;"/></span>`;
                html += `</div>`;
                html += `<table style="width:100%;font-size:10px;border-collapse:collapse;">`;
                html += `<tr style="background:#f0f2f5;"><th style="padding:2px 4px;width:50px;">Campo</th><th style="padding:2px 4px;width:35px;">Min ≥</th><th style="padding:2px 4px;width:35px;">Max ≤</th><th style="padding:2px 4px;width:25px;"></th></tr>`;
                for (const f of FIELDS) {
                    const rule = (gCfg.rules || {})[f.key] || {};
                    const minVal = rule.min !== undefined && rule.min !== null ? rule.min : '';
                    const maxVal = rule.max !== undefined && rule.max !== null ? rule.max : '';
                    html += `<tr style="border-bottom:1px solid #f0f0f0;">`;
                    html += `<td style="padding:2px 4px;font-weight:600;color:${color};">${f.label}</td>`;
                    html += `<td style="padding:2px 4px;"><input type="number" data-group="${gName}" data-field="${f.key}" data-bound="min" value="${minVal}" style="width:48px;font-size:10px;padding:1px 3px;border:1px solid #ccd;border-radius:2px;text-align:right;"/></td>`;
                    html += `<td style="padding:2px 4px;"><input type="number" data-group="${gName}" data-field="${f.key}" data-bound="max" value="${maxVal}" style="width:48px;font-size:10px;padding:1px 3px;border:1px solid #ccd;border-radius:2px;text-align:right;"/></td>`;
                    html += `<td style="padding:2px 4px;color:#999;font-size:9px;">${f.unit}</td>`;
                    html += `</tr>`;
                }
                html += `</table></div>`;
            }
        ...
        cfgSave.addEventListener('click', () => {
            const rules = loadGroupRules();
            // Leer prioridades
            cfgGroups.querySelectorAll('input[data-field="__priority"]').forEach(inp => {
                const g = inp.getAttribute('data-group');
                if (rules[g]) rules[g].priority = parseInt(inp.value) || 99;
            });
            // Leer min/max de cada campo
            cfgGroups.querySelectorAll('input[data-bound]').forEach(inp => {
                const g = inp.getAttribute('data-group');
                const f = inp.getAttribute('data-field');
                const bound = inp.getAttribute('data-bound');
                if (!rules[g]) return;
                if (!rules[g].rules) rules[g].rules = {};
                if (!rules[g].rules[f]) rules[g].rules[f] = {};
                const val = inp.value.trim();
                if (val === '') {
                    delete rules[g].rules[f][bound];
                    // Si ambos estan vacios, eliminar la regla
                    if (Object.keys(rules[g].rules[f]).length === 0) delete rules[g].rules[f];
                } else {
                    rules[g].rules[f][bound] = parseFloat(val);
                }
            });
            saveGroupRules(rules);
            cfgOk.style.display = 'inline';
            setTimeout(() => { cfgOk.style.display = 'none'; }, 2500);
            console.log('[BCN8 Soc] Reglas de grupo guardadas:', JSON.stringify(rules, null, 2));
        });

        cfgReset.addEventListener('click', () => {
            if (!confirm('¿Restaurar los umbrales por defecto? Se perderán los cambios.')) return;
            saveGroupRules(JSON.parse(JSON.stringify(DEFAULT_GROUP_RULES)));
            renderCfgUI();
            cfgOk.textContent = '✓ Restaurado';
            cfgOk.style.display = 'inline';
            setTimeout(() => { cfgOk.style.display = 'none'; cfgOk.textContent = '✓ Guardado'; }, 2500);
        });
